...
#include <dirent.h>
#include <pthread.h>
#include "../header/struct.h" //these are header files//

extern struct examcenter *heade; //this is the linked list//
extern struct examcenter *taile;

 

void* insertexamdata(void* str) //here inserting the examdata function//
{	
	...
	struct examcenter *newitem = calloc(1, sizeof(struct examcenter)); //here dynamically allocating the memory for examcenter//
	...
	pthread_mutex_init(&newitem->lock, NULL); //using pthread_mutex_init() function//
